...
const TopicEditor = forwardRef((props: EditTopicModalProps & { transition: (arg0: EditTopicModalState) => void }, ref: ForwardedRef<HTMLDivElement>) => {
  ...
  useEffect(() => {
    if (props.topic?.title !== undefined) setTitle(props.topic.title)
    if (props.topic?.description !== undefined) setDescription(props.topic.description)
  ...
  return (
    <div ref={ref}>
      {(props.topic !== null)
        ? (
          <>
            <X onClick={() => props.handleClose(false)} className='float-right' />
            <div className='text-center p-2'>
              <input className='text-2xl text-center' type='text' value={title} onChange={updateTitle} name='title' />
            </div>

            <div className='flex flex-row items-center'>
              <p>Description</p>
              <textarea className='grow m-2 border resize-none p-2' value={description} onChange={e => setDescription(e.target.value)} />
            </div>

            <hr className='bg-black m-4' />

            <LexicalTopic
              mode='edit' state={props.topic.content === ''
