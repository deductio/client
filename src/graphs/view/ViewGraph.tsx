...
const ViewGraph = (): JSX.Element => {
  ...
  const openTopic = useCallback((id: number) => {
    ...
    if (!isAvailable(id, progressRef.current, graph.requirements) && topic != null) {
  ...
  return (
    <>
      ...
      <TopicModal topic={openedTopic} handleClose={() => setOpenTopic(null)} dispatch={queueUpdate} completed={progressRef.current.includes(openedTopic?.id ?? -1)} />
      <LockedTopicModal topic={lockedModalTopic} handleClose={() => setLockedModalTopic(null)} />
    </>
  )
}
