...
export class ImageNode extends DecoratorNode<ReactNode> {
    ...
    setUpdate(width: number) {
        const writeable = this.getWritable()

        writeable.__width = width
    }

    setWidth(editor: LexicalEditor, width: number) {
        editor.update(() => {
    ...
    decorate(editor: LexicalEditor): ReactNode {
        if (!editor.isEditable()) {
            ...
            return <Resizable size={{ width: this.__width }} lockAspectRatio onResizeStop={(_e, _dir, _ref, d) => this.setWidth(editor, this.__width + d.width)}>
                <img className="min-w-full w-auto" src={this.__link} onLoad={e => {
                    if (this.__width == 0) this.setWidth(editor, e.currentTarget.offsetWidth)
                }}/>
            </Resizable>
        }
    }
