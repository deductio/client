...
const SearchResult = ({ graph, timerange }: { graph: SearchResultGraph, timerange?: string }): JSX.Element => {
  let range

  const username = Cookies.get('name')
  const fetcher = useFetcher()
  const liked = (fetcher.formData != null) ? fetcher.formData.get('liked') === 'yes' : graph.liked
  ...
  return (
    <div className='m-4'>
      ...
      <i className='flex flex-row'>by
        <Link className='flex flex-row mx-1' to={`/users/${graph.user.username}`}>
          <img className='h-4 rounded-2xl place-self-center mx-0.5' src={graph.user.avatar} />
      ...
      {username !== null
        ? (
          <fetcher.Form method='POST' action={`/like/${graph.id}`} className='float-right'>
            <button
              type='submit' name='liked' value={liked ? 'yes' : 'no'} className={!liked
